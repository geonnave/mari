...
typedef enum {
    ...
    MR_IPC_MIRA_GATEWAY_TX_REQ,  ///< Request to send a packet via the mira drv
    MR_IPC_MIRA_EVENT_REQ,       ///< Request to send a mira event to the application core
...
typedef struct __attribute__((packed)) {
    mr_event_t      event;
    mr_event_data_t data;
} mr_ipc_event_t;

typedef struct __attribute__((packed)) {
    bool               net_ready;  ///< Network core is ready
    bool               net_ack;    ///< Network core acked the latest request
    mr_ipc_req_t       req;        ///< IPC network request
    mr_ipc_rng_data_t  rng;        ///< Rng share data
    mr_ipc_radio_pdu_t tx_pdu;     ///< TX PDU
    mr_ipc_radio_pdu_t rx_pdu;     ///< RX PDU
} mr_ipc_shared_data_t;
